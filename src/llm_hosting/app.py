...
def _split_fallback(text: str) -> Tuple[str, str]:
    ...
    if uni_lower in quick_map:
        uni = quick_map[uni_lower]
        
    prog = re.sub(r'(?i)\bcs\b', 'Computer Science', prog or "")
    prog = re.sub(r'(?i)\bds\b', 'Data Science', prog or "")
    uni = re.sub(r'(?i)\buniv\.?\b', 'University', uni or "")
...
def _cli_process_file(
    ...
    try:
        for row in rows:
            ...
            result = {"standardized_program": prog, "standardized_university": uni}
            row["llm-generated-program"] = result["standardized_program"]
            row["llm-generated-university"] = result["standardized_university"]
